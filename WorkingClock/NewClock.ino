...
void UpdateMode()
{
    ...
    else
    {
        if (_systemDataState > NUMBER_OF_STATE)
        {
            ...
            if (_currentMinute != time.min)
            {
                uint8_t numberchange = 1;
                if(time.min % 10 == 0){
                    numberchange = 2;
                }
                if(time.min == 0){
                ...
                MinuteChangeEffect(numberchange);
            }
            else{
                NoEffect();
            }
...
void MinuteChangeEffect(uint8_t numberofState)
{
    if (_currentEffectState < 9)
    {
        for (uint8_t i = (numberofState - 5) * 6 + 1; i < 32; i++)
        {
            _data[i] = _data[i] << 1  | _nextData[i]<<(8-_currentEffectState);
        }
        _currentEffectState++;
    }
    else
    {
        ...
        _currentMinute = rtc.getTime().min;
        NextEffect();
    }
}
...
void RowTransition()
{
    ...
    else if (_currentEffectState == 40)
    {
        NextEffect();
    }
}
...
void ScanTwice()
{
    ...
    else if (_currentEffectState == 60)
    {
        NextEffect();
    }
}
...
void FlyIn()
{
    ...
    else if (_currentEffectState == 80)
    {
        NextEffect();
    }
}
...
void ReadData()
{
    ...
    else if (_systemDataState == 5)
    {
        ...
        uint8_t d = tpInt % 10;
        Insert2_data(0, c, true);
        Insert2_data(1, d, true);
